...
class SqlJudgeAgent(BaseAgent):
    ...
    def run(self, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        ...
        为了兼容旧版/多样化的调用方式，这里支持：

        - 位置参数形式：run(user_query, sql, schema, db)
        - 关键字参数形式：run(user_query=..., sql_generated=..., schema=..., db=...)
        - 混合调用：run(user_query, sql, schema=..., db=...)

        额外的关键字参数会被忽略，但不会抛出异常，以免上游调用因接口
        扩展而出错。
        """

        user_query = kwargs.pop("user_query", None)
        sql_generated = kwargs.pop("sql_generated", None)
        if sql_generated is None and "sql" in kwargs:
            sql_generated = kwargs.pop("sql")
        schema = kwargs.pop("schema", None)
        db = kwargs.pop("db", None)

        if len(args) > 0 and user_query is None:
            user_query = args[0]
        if len(args) > 1 and sql_generated is None:
            sql_generated = args[1]
        if len(args) > 2 and schema is None:
            schema = args[2]
        if len(args) > 3 and db is None:
            db = args[3]

        if user_query is None:
            raise TypeError("SqlJudgeAgent.run() missing required argument: 'user_query'")
        if sql_generated is None:
            raise TypeError("SqlJudgeAgent.run() missing required argument: 'sql_generated'")

        # 其余 kwargs 保留但当前未使用，仅为了向后兼容
        try:
