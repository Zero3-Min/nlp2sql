...
sql = "SELECT `hospital_id`, `dept_id`, COUNT(*) AS `doctor_count` FROM `testdb`.`doctor_info` GROUP BY `hospital_id`, `dept_id` HAVING (`hospital_id`, COUNT(*)) IN (SELECT `hospital_id`, MAX(COUNT(*)) OVER (PARTITION BY `hospital_id`) FROM `testdb`.`doctor_info` GROUP BY `hospital_id`, `dept_id`);"
